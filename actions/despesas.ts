...
export async function listarDespesas(): Promise<Despesa[]> {
  ...
  const [
    ...
    { data: perfis, error: errorPerfis },
    { data: movimentacoes, error: errorMovimentacoes },
  ] = await Promise.all([
  ...
  if (errorDespesas || errorPerfis || errorMovimentacoes) {
...
export async function obterResumoFinanceiro(): Promise<ResumoFinanceiro> {
  ...
  const [{ data: vendas, error: errorVendas }, { data: despesas, error: errorDespesas }] =
    await Promise.all([
      supabase
        .from("vendas")
